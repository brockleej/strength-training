...
final class CloudKitSyncService {
    /// Must match the iCloud container in `RockLog.entitlements`
    /// and `ModelConfiguration(cloudKitDatabase:)` in `RockLogApp`.
    ...
    /// `ModelConfiguration(cloudKitDatabase:)` in `RockLogApp`.
